...
function setFactoryScores(factory, scores) {
  return {
    type: 'SET_FACTORY_SCORES',
    factory,
    scores,
  };
}
...
export function getFactoryActionPlans(factory, currentPage, pageSize) {
  return function thunk(dispatch) {
    const url = `/api/factories/${factory}/action-plans?pageNo=${currentPage}&size=${pageSize}`;
    sendGetRequest(url, (json) => {
      dispatch(setFactoryActionPlans(factory, json.result));
    });
  };
}
...
export function getFactoryScores(factory, currentPage, pageSize) {
  return function thunk(dispatch) {
    const url = `/api/factories/${factory}/scores?pageNo=${currentPage}&size=${pageSize}`;
    sendGetRequest(url, (json) => {
      dispatch(setFactoryScores(factory, json.result));
    });
  };
}
